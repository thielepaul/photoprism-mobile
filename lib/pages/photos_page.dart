...
class PhotosPage extends StatelessWidget {
  const PhotosPage({Key key, this.albumId, this.videosOnly = false})
      : super(key: key);
  ...
  static Future<void> archiveSelectedPhotos(
      BuildContext context, bool videosOnly) async {
    final PhotoprismModel model = Provider.of<PhotoprismModel>(context);
    final List<String> selectedPhotos = model
        .gridController.selection.selectedIndexes
        .map<String>((int element) =>
            PhotoManager.getPhotos(context, null, videosOnly)[element].uid)
        .toList();

    PhotoManager.archivePhotos(context, selectedPhotos);
  }

  static void _selectAlbumBottomSheet(BuildContext context, bool videosOnly) {
    final PhotoprismModel model = Provider.of<PhotoprismModel>(context);
          ...
          return ListView.builder(
              ...
              itemBuilder: (BuildContext ctxt, int index) {
                return ListTile(
                  ...
                  onTap: () {
                    addPhotosToAlbum(index, context, videosOnly);
                  },
                );
              });
        });
  ...
  static Future<void> addPhotosToAlbum(
      int albumId, BuildContext context, bool videosOnly) async {
  ...
  Widget displayPhotoIfUrlLoaded(BuildContext context, int index) {
    ...
    final String imageUrl =
        PhotoManager.getPhotoThumbnailUrl(context, index, albumId, videosOnly);
    if (imageUrl == null) {
      PhotoManager.loadPhoto(context, index, albumId, videosOnly);
      return Container(
  ...
  static AppBar appBar(BuildContext context, bool videosOnly) {
    final PhotoprismModel model = Provider.of<PhotoprismModel>(context);
      ...
      actions: model.gridController.selection.selectedIndexes.isNotEmpty
          ? <Widget>[
              IconButton(
                ...
                onPressed: () {
                  archiveSelectedPhotos(context, videosOnly);
                },
              ),
              IconButton(
                ...
                onPressed: () {
                  _selectAlbumBottomSheet(context, videosOnly);
                },
              ),
              IconButton(
  ...
  Widget build(BuildContext context) {
    ...
    final int tileCount = videosOnly
