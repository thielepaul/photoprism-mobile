...
class PhotoManager {
  ...
  static Map<int, Photo> getPhotos(
      BuildContext context, int albumId, bool videosOnly) {
  ...
  static Future<void> saveAndSetPhotos(BuildContext context,
      Map<int, Photo> photos, int albumId, bool videosOnly) async {
    final PhotoprismModel model = Provider.of<PhotoprismModel>(context);
    if (videosOnly) {
  ...
  static Future<void> loadPhoto(
      BuildContext context, int index, int albumId, bool videosOnly,
      {bool forceReload = false}) async {
    final PhotoprismModel model = Provider.of<PhotoprismModel>(context);
    return await model.photoLoadingLock.synchronized(() async {
      ...
      if (getPhotos(context, albumId, videosOnly).containsKey(index) &&
          !forceReload) {
        return;
      }
      ...
      if (forceReload) {
        ...
        photos = getPhotos(context, albumId, videosOnly);
      }
      photos.addAll(await Api.loadPhotos(context, albumId, offset,
          videosOnly: videosOnly));
      saveAndSetPhotos(context, photos, albumId, videosOnly);
      return;
    });
  }
  ...
  static String getPhotoThumbnailUrl(
      BuildContext context, int index, int albumId, bool videosOnly) {
    if (getPhotos(context, albumId, videosOnly)[index] == null) {
      return null;
    }
    final PhotoprismModel model = Provider.of<PhotoprismModel>(context);
    final String filehash =
        PhotoManager.getPhotos(context, albumId, videosOnly)[index].hash;
